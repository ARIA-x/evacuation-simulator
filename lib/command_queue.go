...
func (q CommandQueue) EnqueueWriteBuffer(buffer Buffer, blocking bool, dataPtr interface{}) error {
	var br C.cl_bool
	if blocking {
		br = C.CL_TRUE
	} else {
	...
	errInt := clError(C.clEnqueueWriteBuffer(q.commandQueue, buffer.buffer, br, 0, C.size_t(dataLen), ptr, 0, nil, nil))
	return clErrorToError(errInt)
}
